...
* stores a `GameManager` object that represents one game session.
* stores a `Deck` object that represents the deck that the user is viewing when user is in deck view.
* stores a `Card` object that represents the card that the user is playing with when user is in play mode.
* stores a `View` object that represents the view that the user is currently in.
* stores a `Mode` object that represents the mode that the user is currently in.
...
As the UI will have to be constantly updated when we select to view a deck. An
...
A valid `play` command will change the `MODE` of the `ModelManager` to `PLAY` mode and an `GameManager` object will be constructed in `ModelManager`.
