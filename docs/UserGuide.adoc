...
.After deleting the 3rd card in the current deck.
image::delete.png[]
...
.Playing the Japanese deck. Ganbatte kudasai! がんばってください。
image::Ui2.png[]
****
...
Let's flip it to check if your memory is right. +
...
// end::answer-language[]
[NOTE]
You can only answer after flipping the card first.
...
.Statistics shown after manually stopping the session. You will also see it after completing a session.
image::stop.png[]
