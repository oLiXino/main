...
It was developed as a group project for the CS2103T Software Engineering module in the National University of Singapore
(NUS). FlashSpeed was built on top of the AddressBook-Level3 project created by SE-EDU initiative at
https://se-education.org. FlashSpeed is written in Java. Users interact with FlashSpeed using textual commands. FlashSpeed has a GUI created using JavaFX.

== Summary of contributions

My roles in this project includes team leader and in-charge of the User Guide. As team leader my responsibilities included
ensuring the establishment of the week's tasks, adherence to the proper development workflow, and quality checks on pull requests.
...
** Highlights: This is the only command in the application which can take in multiple formats. The implementation of this feature required design considerations across the entire model of the application (e.g. overwrite values in the Card or do a full replacement with a new Card, where should the new Card interface with the Deck to retrieve the unchanged values if any, etc.) In the end, the edit functionality fulfilled its responsibilities while being able to be treated as another standard command.

* *Minor enhancement*: Added the implementations for the functional units of the application, i.e. Cards, Decks, and its UniqueCardList.
** Highlights: These are the abstractions of data which makes FlashSpeed a flashcard application. They are built in such a way that developers of the higher level logic and model management can treat them as total black boxes. These are also the items which users will be mostly interacting with.
...
*** Wrote either fully or partially all sections
...
include::../UserGuide.adoc[tag=amrl-ppp-1]
---
include::../UserGuide.adoc[tag=amrl-ppp-2]
---
include::../UserGuide.adoc[tag=amrl-ppp-3]
---
include::../UserGuide.adoc[tag=amrl-ppp-4]
...
include::../DeveloperGuide.adoc[tag=amrl-ppp-1]
---
